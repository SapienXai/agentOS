...
export default async function DashboardPage({ params }: { params: Promise<{ project_id: string }> }) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        <KpiCard title="System Health" value="Operational" trend="100% uptime" icon={CheckCircle2} trendColor="text-green-500" />
        <KpiCard title="Active Workers" value="8/12" trend="+2 added" icon={Cpu} trendColor="text-blue-500" />
        <KpiCard title="Active Playbooks" value="3" trend="Running now" icon={Zap} trendColor="text-yellow-500" />
        <KpiCard title="Total Runs (24h)" value="1,284" trend="+12% vs yesterday" icon={Activity} trendColor="text-muted-foreground" />
      </div>
...
function KpiCard({ title, value, icon: Icon, trend, trendColor }: { title: string; value: string; icon: ElementType; trend: string; trendColor: string }) {
  return (
    <Card>
...
function WorkerStatus({ name, load, status }: { name: string; load: number; status: "busy" | "idle" | "offline" }) {
  return (
    <div className="space-y-2">
      ...
      <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-500 ${load > 80 ? "bg-red-500" : "bg-primary"}`} style={{ width: `${load}%` }} />
      </div>
    </div>
  );
}
